...
namespace LogoKaresz
{
	public class Avatar
	{
		public Pont Hely { get; private set; }
		...
		{
			Pont hollesz = Hely - new Pont(irány, t, "polár"); // e most polárkoordinátás kellene legyen!
			if (rajzole)
		...
		public Point Hol_vagyok() => Hely.ToPoint();
		...
		private void Kitöltés_mélységi_bejárással(int x, int y, Color mit, Color mire)
		{
			...
			while (tennivalók.Count != 0)
			{
				Point p = tennivalók.Pop();
				Point[] szomszédok = new Point[4]
					{
						new Point(p.X, p.Y + 1),
						new Point(p.X - 1, p.Y),
						new Point(p.X, p.Y - 1),
